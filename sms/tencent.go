...
var (
	ErrUninitialized         = stdErrors.New("sms 未初始化")
	ErrPhoneNumberDailyLimit = stdErrors.New("当日手机号接收短信数量超出限制，请于24小时后重试")
)

func SmsTo(mobile string, tpl string, params []string) (string, error) {
	if smsConf == nil {
		return "", ErrUninitialized
	}

	return TencentSMS(smsConf, mobile, tpl, params)
}

var smsConf *TencentConfig

func Init(config TencentConfig) error {
	smsConf = &config
	return nil
}
...
type TencentConfig struct {
	...
	Endpoint  string `json:"endpoint" toml:"endpoint"` // "sms.tencentcloudapi.com"
	Region    string `json:"region" toml:"region"`     // "ap-guangzhou"
}

func TencentSMS(config *TencentConfig, mobile string, tpl string, params []string) (string, error) {
	// 必要步骤：
	...
	case "LimitExceeded.PhoneNumberDailyLimit":
		return string(b), ErrPhoneNumberDailyLimit
	}
	// 打印返回的json字符串
	 ...
	 */
	fmt.Println(string(b), *response.Response.SendStatusSet[0].Code)
	return string(b), stdErrors.New("短信发送失败，请添加QQ群798312770反馈")
}
